...
import { Center, Environment, View, Float, OrbitControls } from "@react-three/drei";
...
// A simple floating can group that moves in 3D space
...
export default function AboutPage() {
  ...
  useGSAP(() => {
    ...
    // Paragraphs: fade up with stagger, plus a subtle color shift
    gsap.fromTo(
    ...
    // Cards: 3D rotation + scale on scroll
  ...
  return (
    <Bounded className="relative bg-yellow-300 py-16 overflow-hidden">
      {/* 3D Background – floating cans, bubbles, and a skydive-like ambient scene */}
      <div className="absolute inset-0 pointer-events-none z-0">
        <View className="w-full h-full">
          <Canvas camera={{ position: [0, 0, 12], fov: 45 }}>
            ...
            {/* Skydive effect – drifting cans in different positions */}
            ...
            {/* Bubbles rising – reuse from Hero */}
            <Bubbles speed={1.2} count={40} />

            {/* Optional: a subtle rotating light */}
      ...
      {/* Content overlay – with backdrop blur for readability */}
      <div ref={containerRef} className="relative z-10 max-w-4xl mx-auto text-center">
        ...
        <div className="mt-8 text-xl text-sky-900 space-y-6 bg-yellow-300/70 backdrop-blur-sm rounded-2xl p-6">
          <p ref={(el) => { if (el) paragraphsRef.current[0] = el; }} className="leading-relaxed">
            Fizzi was born from a simple idea: create a soda that tastes amazing,
            uses real fruit, and has nothing artificial. No high fructose corn syrup,
            no aspartame – just pure, sparkling refreshment.
          </p>
          <p ref={(el) => { if (el) paragraphsRef.current[1] = el; }} className="leading-relaxed">
          ...
          <p ref={(el) => { if (el) paragraphsRef.current[2] = el; }} className="leading-relaxed">
      ...
      {/* Value cards – now with colorful borders and subtle tilt animation */}
      <div className="relative z-10 mt-20 grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
        {[
          ...
          <div
            ...
            className="rounded-2xl border-2 bg-yellow-300/50 backdrop-blur-md p-6 transition-all cursor-pointer"
            style={{ borderColor: card.color }}
          >
            <div className="text-5xl mb-3 transition-all duration-300" style={{ textShadow: `0 0 5px ${card.color}` }}>
              {card.icon}
            </div>
